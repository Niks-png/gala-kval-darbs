...
if (!file_exists($cacheFolder)) mkdir($cacheFolder, 0755);

$rss_url = "http://feeds.bbci.co.uk/sport/football/rss.xml";
$cacheFileRSS = "$cacheFolder/premier_league_rss.json";
$cacheTimeRSS = 10 * 60; 

function getCachedRSS($rss_url, $cacheFile, $cacheTime) {
...
$newsItems = getCachedRSS($rss_url, $cacheFileRSS, $cacheTimeRSS);

$keywords = ['Premier League','EPL','Manchester','Liverpool','Chelsea','Arsenal','Man United','Tottenham','Leeds','Everton','Newcastle','Wolves','Southampton','Brighton','Crystal Palace','West Ham','Aston Villa','Leicester','Bournemouth','Fulham','Brentford'];

$newsItems = array_filter($newsItems, function($item) use ($keywords) {
    foreach ($keywords as $kw) {
        if (stripos($item['title'], $kw) !== false) return true;
    }
    return false;
});
...
$cacheFileStandings = __DIR__ . "/cache/pl_standings.json";
$cacheTimeStandings = 15 * 60;

if (isset($_GET['refresh']) && file_exists($cacheFileStandings)) unlink($cacheFileStandings);

if (file_exists($cacheFileStandings) && time() - filemtime($cacheFileStandings) < $cacheTimeStandings) {
    $data = json_decode(file_get_contents($cacheFileStandings), true);
} else {
    $ch = curl_init("https://api.football-data.org/v4/competitions/PL/standings");
    ...
    curl_close($ch);
    file_put_contents($cacheFileStandings, $response);
...
<head>
    <meta charset="UTF-8">
    <title>Footy Premier League</title>
...
<div class="dashboard-content">
    ...
    <div class="viss">
        <div class="virsraksts"> <img src="images/footy.png"  width="130" height="80"></div>
    ...
    <div class="bigbox">
        <div class="news-container">
            <h2>⚽ Premier League Latest News</h2>
            <div class="news-list">
            ...
            <div class="standings-container">
                ...
                <div class="table-wrapper">
                    <table class="standings">
                        <thead>
                            <tr>
                                <th>#</th><th>Club</th><th>MP</th><th>W</th><th>D</th><th>L</th><th>GD</th><th>Pts</th>
                            </tr>
                        </thead>
                        <tbody>
