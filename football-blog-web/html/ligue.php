...
$apiKey = "e1271738e8454dbd827d3f58cee8aee8";
$cacheFolder = __DIR__ . "/cache";
if(!file_exists($cacheFolder)) mkdir($cacheFolder,0755,true);

$cacheFileNews = "$cacheFolder/ligue1_news.json";
$cacheTimeNews = 5*60; 
...
if(file_exists($cacheFileNews) && time()-filemtime($cacheFileNews)<$cacheTimeNews){
    $data = json_decode(file_get_contents($cacheFileNews), true);
}else{
    $ch = curl_init($url);
    ...
    if($httpcode != 200) die("Failed to fetch news data. HTTP Code: $httpcode");

    $data = json_decode($response, true);
    file_put_contents($cacheFileNews, json_encode($data));
}

$newsItems = array_slice($data['articles'] ?? [], 0, 5);

$apiKey = "063f0699e14d434b94fbaa277a38e2f2";
$cacheFileStandings = __DIR__ . "/cache/FL1_standings.json";
...
if (file_exists($cacheFileStandings) && time() - filemtime($cacheFileStandings) < $cacheTimeStandings) {
    ...
    $ch = curl_init("https://api.football-data.org/v4/competitions/FL1/standings");
...
<div class="dashboard-content">
    ...
    <div class="bigbox">
        <div class="news-container">
            <h2>⚽ Ligue 1 Latest News</h2>
            <div class="news-list">
                <?php if(empty($newsItems)): ?>
                    <p>No Ligue 1 news available at the moment.</p>
                <?php else: ?>
                    <ul>
                    <?php foreach($newsItems as $item):
                        $title = htmlspecialchars($item['title']);
                        $link = htmlspecialchars($item['url']);
                        $date = date("d M Y", strtotime($item['publishedAt']));
                    ?>
                        <li>
        ...
        <div class="box2">
            <div class="standings-container">
                <h2>🏆 Ligue 1 League Standings</h2>
                <div class="table-wrapper">
                    <table class="standings">
                        ...
                        <?php foreach ($data['standings'][0]['table'] as $team): ?>
                            <tr>
                                <td><?= $team['position'] ?></td>
                                <td class="team"><img src="<?= $team['team']['crest'] ?>" alt=""> <?= $team['team']['name'] ?></td>
